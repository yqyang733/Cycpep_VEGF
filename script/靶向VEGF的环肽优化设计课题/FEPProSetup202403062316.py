...
def do_mutate(pdb, segname, resid, ff_path, mutation, flag, vmd_path):
...
mol new {0}
set sel [atomselect top "segname {1} and resid {2} and not name C N O HN"]
foreach name [$sel get name] {{
    set sel [atomselect top "segname {1} and resid {2} and name $name"]
    $sel set name ${{name}}A
}}
...
def mark_fep(resid):
    ...
    for i in f1:
        if i[66:76].strip() == "MUT" and i[22:26].strip() == str(resid):
...
parameters          ${{parpath}}/toppar_all36_prot_c36m_d_aminoacids.inp
...
def run_fep_individe(pdb, psf, segname, resid, ff_path, mutation, flag, vmd_path, duplicate, submit, subname):

    do_mutate(pdb, segname, resid, ff_path, mutation, flag, vmd_path)
...
def run_fep():
    ...
    settings = config("Script/settings.dat")
    settings.resid = sys.argv[1]
    settings.mutation = sys.argv[2]
    file_name = settings.mutation[0] + str(settings.resid) + settings.mutation[-1]
    ...
    run_fep_individe(settings.bond_pdb, settings.bond_psf, settings.segname, settings.resid, settings.ff_path, settings.mutation, "bonded", settings.vmd_path, settings.duplicate, settings.submit, file_name+"_b")
    os.chdir(os.path.join(".."))

    os.makedirs(os.path.join(".", "free"))
    os.chdir(os.path.join(".", "free"))
    run_fep_individe(settings.free_pdb, settings.free_psf, settings.segname, settings.resid, settings.ff_path, settings.mutation, "free", settings.vmd_path, settings.duplicate, settings.submit, file_name+"_f")
    os.chdir(os.path.join(".."))
