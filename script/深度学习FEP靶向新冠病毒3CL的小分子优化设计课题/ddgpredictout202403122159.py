...
class config:
    ...
    def __init__(self):
        ...
        self.test = os.path.join("./input_pairs_1.csv")
...
def read_data(in_file):
    ...
    for i in f1:
        line = i.strip().split(",")
        rt_data.append([line[0], line[1], 0])
...
def predict_out(test_data, predict):
    ...
    rt.write("Mol_1,Mol_2,ddg_1,ddg_2,ddg_3,ddg_4,ddg_5,ddg_6,ddg_7,ddg_8,ddg_9,ddg_10,ddg_mean\n")
    for i in range(len(test_data)):
        rt.write(test_data[i][0]+","+test_data[i][1]+","+str(predict[0][i])+","+str(predict[1][i])+","+str(predict[2][i])+","+str(predict[3][i])+","+str(predict[4][i])+","+str(predict[5][i])+","+str(predict[6][i])+","+str(predict[7][i])+","+str(predict[8][i])+","+str(predict[9][i])+","+str((predict[0][i]+predict[1][i]+predict[2][i]+predict[3][i]+predict[4][i]+predict[5][i]+predict[6][i]+predict[7][i]+predict[8][i]+predict[9][i])/10)+"\n")
    rt.close()
...
def run():
    ...
    y_predict_all = []
    for i in range(1, 11):
        model.load_pretrained("../model/run"+str(i)+"/model_epoch59/model.pt")
        print("Starting predict.")
        y_predict = model.test(test)
        y_predict_all.append(y_predict)
    predict_out(test, y_predict_all)
    print("Predict done.")
