...
class config:
    ...
    def __init__(self):
        ...
        self.reaction_grp = "lig"
        self.target_grp = "Backbone"
        self.time = float(500)   # ns
        self.wins = "submit.dat"
        self.queue = "quick"
        self.name = "aa"
...
define      = -DPOSRES_pro -DPOSRES_FC_BB=1000 -DPOSRES_FC_SC=0.0
; Run parameters
integrator  = md
dt          = 0.002
tinit       = 0
...
cd prod
if [ ! -f prod.gro ];then
    gmx grompp -f ../../mdp/prod.mdp -c ../win_{0}.gro -p ../../topol.top -o prod.tpr -r ../win_{0}.gro -maxwarn 100 -n ../../index.ndx
    gmx mdrun -s prod.tpr -deffnm prod -dhdl dhdl -ntmpi 1 -nb gpu -bonded gpu -pme gpu -gpu_id 0 -pf prodf.xvg -px prodx.xvg -nsteps 25000000 -update gpu
fi
